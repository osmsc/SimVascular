...
cgi_eval {
    ...
    set ConferenceURL http://www.usenix.org/events/tcl98
    set BarURL $EXPECT_ART/pink.gif

    set Q(filename) "data/vote.cnt"
	...
	if {[regexp "Vote(\[0-9]+)" [cgi_import_list] dummy i]} {
	    ...
	    h3 "Thanks for voting!  See you at the [url conference $ConferenceURL]!"
	    set ShowVotes 1
	}
	...
	p "Vote for what will go on the [url "Tcl/Tk Conference" $ConferenceURL] t-shirt!  (Feel free to vote for several entries.)"
