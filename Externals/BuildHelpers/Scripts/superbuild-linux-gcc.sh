...
if [[ $SV_SUPER_OPTIONS == *BUILD_SWIG* ]]; then
  ...
  sed -f CompileScripts/sed-script-x64_linux-options-gcc.sh CompileScripts/compile-make-swig-generic.sh > tmp/compile.make.swig.gcc.sh
...
if [[ $SV_SUPER_OPTIONS == *BUILD_QT* ]]; then
  ...
  sed -f CompileScripts/sed-script-x64_linux-options-gcc.sh CompileScripts/compile-make-qt-generic.sh > tmp/compile.make.qt.gcc.sh
